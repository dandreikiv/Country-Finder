...
- (BOOL)constainsLocation:(CLLocation *)location
{
    MKShape * shape = [self mapkitShape];
    if ([shape conformsToProtocol:@protocol(MKOverlay)]) {
        MKMapRect mapRect = [(id<MKOverlay>)shape boundingMapRect];
        MKMapPoint mapPoint = MKMapPointForCoordinate(location.coordinate);
        NSLog(@"%@", MKStringFromMapPoint(mapPoint));
        BOOL result = MKMapRectContainsPoint(mapRect, mapPoint);
        return result;
    }
